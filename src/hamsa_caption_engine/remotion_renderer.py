...
from .paths import LOG_DIR, REMOTION_DIR, ROOT, find_executable
...
def _append_remotion_log(
    *,
    ...
    logo_path = ROOT / "assets" / "brand" / "hamsa-logo.png"
    ...
    lines = [
        ...
        f"cwd: {REMOTION_DIR}",
        f"assets/brand/hamsa-logo.png exists: {logo_path.exists()}",
    ]
    if error:
        lines.extend(["preflight error:", error])
    if proc:
        lines.extend([
            "stdout:",
            proc.stdout or "",
...
def _preflight(input_path: Path, recipe_path: Path, node: str | None, render_script: Path) -> str | None:
    if not node:
    ...
    if not recipe_path.exists():
        return f"Recipe file does not exist: {recipe_path}"
    return None
...
def render_remotion(video_path: str | Path, output_dir: str | Path, recipe: dict[str, Any]) -> dict[str, Path]:
    ...
    recipe_path = save_recipe(recipe, (out / "edit_recipe.json").resolve()).resolve()
    ...
    preflight_error = _preflight(input_path, recipe_path, node, render_script)
    if preflight_error:
        _append_remotion_log(cmd=cmd, node_path=node, node_version=node_version, npm_path=npm, npm_version=npm_version, render_script=render_script, input_path=input_path, recipe_path=recipe_path, output_path=output_path, error=preflight_error)
    ...
    if proc.returncode != 0:
        _append_remotion_log(cmd=cmd, node_path=node, node_version=node_version, npm_path=npm, npm_version=npm_version, render_script=render_script, input_path=input_path, recipe_path=recipe_path, output_path=output_path, proc=proc)
    ...
    warning = node_version_warning(node_version)
    if warning:
        _append_remotion_log(cmd=cmd, node_path=node, node_version=node_version, npm_path=npm, npm_version=npm_version, render_script=render_script, input_path=input_path, recipe_path=recipe_path, output_path=output_path, proc=proc, error=warning)
    if not thumbnail_path.exists():
        # Remotion script renders still when supported; absence is not fatal.
        thumbnail_path.touch()
    return {"final_video": output_path, "thumbnail": thumbnail_path, "recipe": recipe_path}
